...
namespace adam 
{
    ...
    static ADAM_CONSTEXPR uint8_t get_major(uint32_t ver) { return static_cast<uint8_t>((ver >> 16) & 0xFF); }
    static ADAM_CONSTEXPR uint8_t get_minor(uint32_t ver) { return static_cast<uint8_t>((ver >> 8) & 0xFF); }
    static ADAM_CONSTEXPR uint8_t get_patch(uint32_t ver) { return static_cast<uint8_t>(ver & 0xFF); }

    // Helper to decode into a struct
    static ADAM_CONSTEXPR version_info decode_version(uint32_t ver) 
    {
        return { get_major(ver), get_minor(ver), get_patch(ver) };
    }

    // Helper to encode (useful for internal logic or comparison)
    static ADAM_CONSTEXPR uint32_t make_version(uint8_t maj, uint8_t min, uint8_t pat) 
    {
