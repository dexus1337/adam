...
namespace adam::modules::can
{
     ...
     */
    inline uint64_t extract_raw_signal(const uint8_t* payload, uint8_t dlc, const can_signal_spec& spec)
    {
        if (!payload || spec.bit_offset >= static_cast<uint16_t>(dlc * 8) || spec.bit_length == 0)
        {
            return 0;
        }
    ...
    public:
        ...
        {
            if (!is_extended && can_id < 2048)
            {
                return m_std_id_table[can_id];
            }

            auto it = m_extended_id_table.find(can_id);
            if (it != m_extended_id_table.end())
            {
                return it->second;
            }
