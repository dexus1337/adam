...
namespace adam::modules::network
{
    ...
    bool port_udp_unicast::start()
    {
        ...
        {
            ctrl->log(log::error, std::format("[{}] UDP-Unicast: {} ({})", get_name().c_str(), get_event_log_text(log_event::socket_bind_failed, lang), get_error_log_text(socket_error_t::error_address_invalid, lang), m_local_interface->get_value().c_str()));
            return false;
        }
        ...
        ctrl->log(log::info, std::format("[{}] UDP-Unicast: {} (Port {})", get_name().c_str(), get_event_log_text(log_event::socket_bind_success, lang), m_local_port->get_value()));
