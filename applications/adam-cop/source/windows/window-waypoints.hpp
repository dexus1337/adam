#pragma once

/**
 * @file    window-waypoints.hpp
 * @author  dexus1337
 * @brief   Header for the waypoints management window in adam-cop.
 * @version 1.0
 * @date    20.08.2026
 */

#include <adam-sdk.hpp>

namespace adam::cop
{
    class cop_controller;
    class world_map;

    /**
     * @brief Renders the Waypoints management window.
     * @param ctrl Reference to the COP controller.
     * @param map Reference to the world map instance.
     * @param lang Current active user language.
     * @param p_show_waypoints Configuration parameter controlling window visibility.
     */
    void draw_waypoints_window(cop_controller& ctrl, world_map& map, adam::language lang, adam::configuration_parameter_boolean* p_show_waypoints);
}
