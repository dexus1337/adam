        ...
        for (const auto& [hash, p_view] : ports)
        {
            if (std::find(g_expanded_inspector_ports.begin(), g_expanded_inspector_ports.end(), hash) != g_expanded_inspector_ports.end())
            {
                bool has_data = false;
                {
                    ...
                    if (!adam::gui::g_inspection_data.buffers[hash].empty()) has_data = true;
                }
        ...
        if (table_open)
        {
            ...
            for (adam::string_hash port_hash : port_hashes)
            {
                ...
                if (node_open)
                {
                    ...
                    bool table_begun = ImGui::BeginTable("InspectorTableInner", 6, ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_BordersOuter | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollY | ImGuiTableFlags_ScrollX, ImVec2(0, inspector_height));
