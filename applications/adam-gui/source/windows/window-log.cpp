...
namespace adam::gui 
{
    static void draw_log_sort_control
    (
        adam::language lang,
        float dpi_scale,
        int& sort_mode,
        adam::configuration_parameter_integer* sort_mode_param
    )
    {
    ...
    static void draw_log_search_bar
    (
        adam::language lang,
        float right_controls_offset,
        std::string& search_str,
        adam::configuration_parameter_string* search_param
    )
    {
        ...
        float right_controls_x = ImGui::GetWindowContentRegionMax().x - right_controls_offset;
    ...
    static void draw_log_level_and_clear_controls
    (
        gui_controller& ctrl,
        adam::language lang,
        float right_controls_x,
        float combo_width,
        const char* clear_log_text,
        adam::configuration_parameter_integer* p_log_level
    )
    {
    ...
    {
        draw_log_sort_control(lang, dpi_scale, sort_mode, sort_mode_param);
        ...
        float btn_width = ImGui::CalcTextSize(clear_log_text, nullptr, true).x + ImGui::GetStyle().FramePadding.x * 2.0f;
        ...
        draw_log_search_bar(lang, right_align_offset, search_str, search_param);
        draw_log_level_and_clear_controls(ctrl, lang, right_controls_x, combo_width, clear_log_text, p_log_level);
    }

    static inline void draw_log_row
    (
        adam::language lang,
        int row_idx,
        const log_entry& log_line
    )
    {
        ImGui::TableNextRow();
        ...
        switch (log_line.level)
        {
            ...
            default:                        color = ImVec4(r, g, b, 1.0f); break;
        }
    ...
    )
    {
        ...
        log_history.reserve(log_history_raw.size());
        for (const auto& log_line : log_history_raw)
        {
            if (!search_lower.empty())
            {
                std::string msg_lower = log_line.text.c_str();
                std::transform(msg_lower.begin(), msg_lower.end(), msg_lower.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        ...
        {
            draw_log_row(lang, row_idx++, log_line);
        }
    ...
    void draw_window_log(gui_controller& ctrl, adam::language lang, int log_table_id)
    {
        ...
        auto* sort_mode_param = dynamic_cast<adam::configuration_parameter_integer*>(ctrl.get_parameters().get("log_sort_mode"_ct));
        int sort_mode = sort_mode_param ? static_cast<int>(sort_mode_param->get_value()) : 0;

        auto* search_param = dynamic_cast<adam::configuration_parameter_string*>(ctrl.get_parameters().get("log_search"_ct));
        std::string search_str = search_param ? std::string(search_param->get_value().c_str()) : "";

        draw_log_toolbar(ctrl, lang, dpi_scale, sort_mode, sort_mode_param, search_str, search_param, p_log_level);
        draw_log_table(ctrl, lang, log_table_id, sort_mode, search_str);
